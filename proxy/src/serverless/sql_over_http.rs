...
use postgres_client::types::Type;
    ...
    async fn process(
        ...
        let status = 'success: {
            let query = pin!(query_to_json(
                ...
                &mut *inner,
                self,
                ValueSer::new(&mut json_output),
                parsed_headers
            ));

            let query = match select(query, pin!(cancel.cancelled())).await {
                // The query successfully completed.
                Either::Left((Ok(status), __not_yet_cancelled)) => break 'success status,
                ...
                Either::Right((_cancelled, query)) => query,
            };
            ...
            match time::timeout(time::Duration::from_millis(100), query).await {
                ...
                Ok(Ok(status)) => break 'success status,
...
impl BatchQueryData {
    async fn process(
        ...
        debug!("starting batch transaction");
        let (inner, mut discard) = client.inner();
        let cancel_token = inner.cancel_token();
...
async fn query_to_json<T: GenericClient>(
    ...
    let (mut statement, mut row_stream) = client
    ...
    for c in statement.columns() {
        ...
        // fix this
        types.push(Type::from_oid(c.type_oid()).expect("just testing for now"));
    ...
    let (command_tag, ready) = row_stream.into_inner();
    info!(
        rows,
        ?ready,
        command_tag,
